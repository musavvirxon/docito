...
async function generateTreatmentPlanPdf(params: {
  ...
  const bottomReserve = 110; // keep space for footer on last page
  ...
  if (!params.procedures.length) {
    ...
    const itemTitleSize = 11;
    const itemLabelSize = 9;

    for (let idx = 0; idx < params.procedures.length; idx++) {
      const p = params.procedures[idx];
      ensureSpace(70);

      const title = formatForLocale(params.locale, `${idx + 1}. ${p.name}`);
      const titleW = font.widthOfTextAtSize(title, itemTitleSize);
      ...
      const teethStr = teeth.length ? teeth.join(", ") : t(params.locale, "na");

      const statusStr = safeText(p.status, t(params.locale, "na"));
      const costStr = safeText(p.cost, t(params.locale, "na"));

      // mini KVs
      const kv = (k: string, v: string) => {
        ensureSpace(14);
        const label = formatForLocale(params.locale, t(params.locale, k));
        const value = formatForLocale(params.locale, v);

        const lx = margin;
        const vx = margin + 140;
        const vw = W - margin - vx;

        const labelW = font.widthOfTextAtSize(label, itemLabelSize);
        const labelX = rtl ? (vx + vw - labelW) : lx;
        ...
        y = res.yAfter + (14 - lineGap);
      };

      kv("tooth", teethStr);
      kv("procedureStatus", statusStr);
      kv("cost", costStr);

      if (p.notes) kv("procedureNotes", p.notes);
  ...
  if (!params.medications.length) {
    ...
    for (let i = 0; i < params.medications.length; i++) {
      ...
      const head = formatForLocale(params.locale, `${i + 1}. ${m.name}`);
      const headSize = 11;
      const headW = font.widthOfTextAtSize(head, headSize);

      page.drawText(head, { x: rtl ? (W - margin - headW) : margin, y, size: headSize, font, color: rgb(0.05, 0.05, 0.05) });
      y -= 14;

      drawKV("dosage", m.dosage);
      drawKV("frequency", m.frequency);
      if (m.instructions) drawKV("instructions", m.instructions);
      if (m.startDate) drawKV("startDate", m.startDate);
      if (m.endDate) drawKV("endDate", m.endDate);
      if (m.status) drawKV("medicationStatus", m.status);
  ...
  // Footer on last page
  // If we don't have enough room for footer, add a new page for footer
...
serve(async (req: Request) => {
  ...
  if (providerId) {
    const { data: doctorRow } = await serviceClient
      .from("doctors")
      .select("id, user_id, specialty, specialty_en, specialty_ru, specialty_uz, specialty_ar")
      .eq("id", providerId)
      .maybeSingle();
