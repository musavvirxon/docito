...
export const generateProfilePatientPDF = async (
  ...
  try {
    toast.loading("Generating patient profile PDF...");

    // Use the new edge function which embeds a scannable QR verification code
    const { supabase } = await import("@/integrations/supabase/client");
    const { data, error } = await supabase.functions.invoke("patient-profile-generate-pdf", {
      body: { patient_id: patientUserId },
    });
    ...
    // Fetch patient name for filename
    let patientName = patientUserId.slice(0, 8);
    try {
      const { data: prof } = await supabase.from("profiles").select("full_name").eq("user_id", patientUserId).maybeSingle();
      if ((prof as any)?.full_name) patientName = (prof as any).full_name;
    } catch { /* ignore */ }
    ...
    a.href = blobUrl;
    a.download = `patient-profile-${patientName.replace(/\s+/g, "-").toLowerCase()}.pdf`;
