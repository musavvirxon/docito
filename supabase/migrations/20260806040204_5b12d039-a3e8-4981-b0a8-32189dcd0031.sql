...
DROP VIEW IF EXISTS public.patient_ledger_v;

CREATE VIEW public.patient_ledger_v
...
SELECT
  ('opening-' || dp.id::text) AS entry_id,
  dp.id AS patient_id,
  NULL::uuid AS practice_id,
  NULL::uuid AS appointment_id,
  COALESCE(dp.opening_balance_date::timestamptz, dp.created_at) AS entry_date,
  'opening'::text AS kind,
  'Opening balance'::text AS description,
  ROUND(dp.opening_balance_amount * 100)::bigint AS charge_cents,
  0::bigint AS payment_cents,
  lower(COALESCE(dp.opening_balance_currency, 'uzs')) AS currency,
  NULL::text AS method
FROM public.doctor_patients dp
WHERE dp.opening_balance_amount IS NOT NULL AND dp.opening_balance_amount <> 0
