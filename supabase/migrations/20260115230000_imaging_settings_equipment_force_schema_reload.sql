-- File: supabase/migrations/20260115230000_imaging_settings_equipment_force_schema_reload.sql
begin;

create extension if not exists pgcrypto;

-- ------------------------------------------------------------
-- imaging_equipment (idempotent)
-- ------------------------------------------------------------
...
create index if not exists idx_imaging_equipment_center_center_modality on public.imaging_equipment(imaging_center_id, modality);

alter table public.imaging_equipment enable row level security;

-- ------------------------------------------------------------
-- imaging_center_settings (idempotent)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- updated_at helper (idempotent) + triggers
-- ------------------------------------------------------------
do $$
begin
  if not exists (
  ...
  if not exists (select 1 from pg_trigger where tgname = 'trg_imaging_center_settings_updated_at') then
    create trigger trg_imaging_center_settings_updated_at
...
-- Force PostgREST schema reload (fixes "schema cache" errors after migrations)
