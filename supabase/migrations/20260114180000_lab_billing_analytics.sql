-- 20260114180000_lab_billing_analytics.sql
...
-- RLS: lab admin OR active lab staff can read/write
...
-- View for dashboard (insurance_providers.provider_name exists in your schema)
...
select
  ...
  coalesce(ip.provider_name, '') as insurance_provider,
  c.policy_number,
  c.claim_amount,
  c.approved_amount,
  c.copay_amount,
...
-- =========================
-- Analytics RPC (Edge Function calls this)
-- =========================
...
begin
  ...
  -- Access control (lab admin OR active staff)
  if not (
    exists (select 1 from public.lab_centers lc where lc.id = p_lab_center_id and lc.admin_id = auth.uid())
    or exists (select 1 from public.lab_staff ls where ls.lab_center_id = p_lab_center_id and ls.user_id = auth.uid() and ls.status = 'active')
  ) then
  ...
  where o.lab_center_id = p_lab_center_id
    ...
    and (o.status = 'completed' or o.completed_at is not null);
  ...
  -- Recollection rate (status like recollect%)
  select
