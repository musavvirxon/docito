-- File: supabase/migrations/20260115131000_imaging_dashboard_live.sql

begin;

-- ============================================================
-- Imaging dashboard: equipment + settings + billing entity support
-- ============================================================
...
-- Keep updated_at in sync (reuse shared helper if present)
do $$
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_imaging_equipment_updated_at'
  ) then
...
exception when undefined_function then
  -- If update_updated_at_column doesn't exist, create a minimal replacement trigger function
...
-- Policies: admins & staff can read; admins & staff with can_manage_equipment can write
...
-- 3) Billing transactions: add entity columns (imaging center billing support)
...
-- Allow imaging center admins/staff to SELECT billing_transactions for their center (entity-scoped)
