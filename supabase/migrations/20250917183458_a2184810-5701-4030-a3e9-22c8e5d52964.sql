-- Continue with remaining database functions (fixed syntax)
...
-- Advanced Functionality (fixed variable name)
  ...
  WHILE check_date <= date_to LOOP
    ...
    WHILE time_slot <= '17:00'::TIME LOOP
      ...
      SELECT public.validate_appointment_slot(
        ...
        check_date, 
        time_slot, 
        (time_slot::INTERVAL + (procedure_duration || ' minutes')::INTERVAL)::TIME
      ...
      IF slot_available THEN
        available_slots := array_append(available_slots, 
          json_build_object(
            ...
            'end_time', (time_slot::INTERVAL + (procedure_duration || ' minutes')::INTERVAL)::TIME::TEXT,
            'available', true
          )
        );
      END IF;
      
      time_slot := (time_slot::INTERVAL + '30 minutes'::INTERVAL)::TIME;
    END LOOP;
