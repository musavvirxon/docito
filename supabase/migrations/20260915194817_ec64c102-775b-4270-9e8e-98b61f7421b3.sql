...
AS $$
DECLARE
  _uid uuid := coalesce(auth.uid(), _requesting_user_id);
...
BEGIN
  ...
  IF _doctor_id IS NOT NULL THEN
    ...
    FROM public.doctors d WHERE d.id = _doctor_id;
    _resolved_practice_id := coalesce(
      _practice_id, _resolved_practice_id,
  ...
  ELSE
    _allowed := _doctor_user_id = _uid
      ...
      OR EXISTS (SELECT 1 FROM public.appointments a WHERE a.practice_id = _resolved_practice_id AND (_doctor_id IS NULL OR a.doctor_id = _doctor_id) AND a.patient_id = _uid);
  END IF;
  IF NOT _allowed THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;

  RETURN QUERY
  WITH selected_branch AS (
    SELECT pl.* FROM public.practice_locations pl
    WHERE pl.practice_id = _resolved_practice_id
    ORDER BY (pl.id = coalesce(_branch_id, _doctor_location_id)) DESC, pl.is_primary DESC NULLS LAST, pl.created_at ASC, pl.id ASC LIMIT 1
  )
  SELECT p.id, p.name::text,
    ...
    sb.id,
    coalesce(CASE _language WHEN 'ru' THEN nullif(sb.name_ru, '') WHEN 'uz' THEN nullif(sb.name_uz, '') WHEN 'en' THEN nullif(sb.name_en, '') WHEN 'ar' THEN nullif(sb.name_ar, '') END, nullif(sb.name, ''), p.name)::text,
    coalesce(CASE _language WHEN 'ru' THEN nullif(sb.address_ru, '') WHEN 'uz' THEN nullif(sb.address_uz, '') WHEN 'en' THEN nullif(sb.address_en, '') WHEN 'ar' THEN nullif(sb.address_ar, '') END, nullif(sb.address, ''), p.address)::text,
    coalesce(nullif(sb.phone, ''), p.phone)::text,
    coalesce(nullif(sb.email, ''), p.email)::text,
    d.id, pr.full_name::text,
    coalesce(CASE _language WHEN 'ru' THEN nullif(d.specialty_ru, '') WHEN 'uz' THEN nullif(d.specialty_uz, '') WHEN 'en' THEN nullif(d.specialty_en, '') WHEN 'ar' THEN nullif(d.specialty_ar, '') END, d.specialty)::text,
...
REVOKE ALL ON FUNCTION public.get_document_branding_v2(uuid, uuid, uuid, text, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_document_branding_v2(uuid, uuid, uuid, text, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.get_document_branding_v2(uuid, uuid, uuid, text, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.get_document_branding_v2(uuid, uuid, uuid, text, uuid) TO service_role;