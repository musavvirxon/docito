-- Create the get_my_entity_scopes RPC function that returns all entities the current user has access to
...
BEGIN
  ...
  SELECT
    ...
    false AS is_admin,
    jsonb_build_object(
      'can_view_orders', COALESCE(ls.can_view_orders, false),
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'can_view_prescriptions', COALESCE(ps.can_view_prescriptions, false),
      'can_process_insurance', COALESCE(ps.can_process_insurance, false)
    ) AS permissions
