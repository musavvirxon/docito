...
export default function PracticeVerification() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Verification request submitted successfully! We'll review your application within 2-3 business days.");
      navigate("/dashboard");
    } catch (error: any) {
                          ...
                          <div className="flex items-center gap-2 mt-3">
                            ...
                            <Button variant="outline" size="sm" onClick={() => {
                              setDocuments(prev => prev.map(d => 
                                d.type === doc.type ? { ...d, uploaded: false, file_path: undefined } : d
                              ));
                            }}>
                              Replace
                            </Button>
                ...
                <div className="flex gap-2 mt-2 mb-3 flex-wrap">
                  {formData.specialties.map((specialty, index) => (
                    <Badge key={index} variant="secondary" className="gap-1">
                      ...
                      <X
                        className="w-3 h-3 cursor-pointer hover:text-destructive"
                        onClick={() => removeItem("specialty", index)}
                      />
                    </Badge>
                  ))}
                </div>
