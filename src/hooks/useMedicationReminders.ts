...
export const useMedicationReminders = () => {
  ...
  const loadReminders = async () => {
    ...
    try {
      ...
      const todaysData = reminderData.filter(r => 
        r.reminder_time.startsWith(today)
      );
      setTodaysReminders(todaysData);

      // Filter upcoming reminders (next 7 days, excluding today)
      const nextWeek = new Date();
      nextWeek.setDate(nextWeek.getDate() + 7);
      const upcomingData = reminderData.filter(r => {
        const reminderDate = new Date(r.reminder_time);
        return reminderDate > new Date(today + 'T23:59:59') && 
