...
export const ROLE_PERMISSIONS: Record<AppRole, string[]> = {
  ...
  doctor: [
    ...
    'lab_orders:write', 'imaging_orders:write',
  ],
  pharmacy_admin: [
    'pharmacy:read', 'pharmacy:write', 'pharmacy:delete',
    'pharmacy_staff:read', 'pharmacy_staff:write', 'pharmacy_staff:delete',
    'prescriptions:read',
  ...
  lab_admin: [
    ...
    'lab_staff:read', 'lab_staff:write', 'lab_staff:delete',
    'lab_orders:read', 'lab_orders:write',
    'lab_results:read', 'lab_results:write',
  ],
  imaging_admin: [
    'imaging:read', 'imaging:write', 'imaging:delete',
    'imaging_staff:read', 'imaging_staff:write', 'imaging_staff:delete',
    'imaging_orders:read', 'imaging_orders:write',
    'imaging_results:read', 'imaging_results:write',
  ],
  pharmacist: [
    'prescriptions:read',
    'inventory:read', 'inventory:write',
...
export function hasPermission(userRoles: string[], permission: string): boolean {
  for (const role of userRoles) {
    ...
    // Check wildcard permissions (e.g., 'clinic:*' matches 'clinic:read')
...
export function getDashboardRoute(roles: string[]): string {
  ...
  if (primaryRole && DASHBOARD_ROUTES[primaryRole]) {
    return DASHBOARD_ROUTES[primaryRole];
  }
