...
const MonthView = memo(
  ({ selectedDate, appointments, scheduleSettings, getScheduleHealth, filters, loading, onDayClick }: MonthViewProps) => {
          ...
          {calendarDays.map((day, index) => {
            ...
            return (
              ...
              >
                ...
                {/* ✅ Appointment indicators now include requested procedure */}
                {isCurrentMonth && isWorking && (
                  <div className="space-y-1">
                    {dayAppts.slice(0, 3).map((apt) => {
                      const firstName = apt.patient_name?.split(' ')?.[0] || 'Patient';
                      const proc = apt.procedure_name ? shorten(apt.procedure_name, 12) : '';
                      const line = proc ? `${apt.start_time} ${firstName} • ${proc}` : `${apt.start_time} ${firstName}`;
                      ...
                      return (
                        <div
                          ...
                          title={apt.procedure_name ? `${apt.start_time} ${firstName} • ${apt.procedure_name}` : undefined}
                        >
                          {line}
                        </div>
                      );
                    ...
                    {dayAppts.length > 3 && (
                      <div className="text-[10px] text-muted-foreground text-center">+{dayAppts.length - 3} more</div>
                    )}
                  </div>
                )}
                ...
                {isCurrentMonth && !isWorking && (
                  <div className="text-[10px] text-muted-foreground mt-2">{t('doctor.calendar.off', 'Off')}</div>
                )}
              </motion.div>
            );
          })}
