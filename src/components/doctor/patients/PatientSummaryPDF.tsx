...
export const generatePatientSummaryPDF = async (
  ...
  try {
    ...
    // Clinic and responsible doctor identity
    const [clinicLogoDataUrl, doctorPhotoDataUrl] = await Promise.all([
      loadImageAsDataUrl(practiceInfo?.logo_url),
      loadImageAsDataUrl(practiceInfo?.doctor_photo_url),
    ]);
...
doc.text(`Generated by: ${doctorName}`, pageWidth - 14, 20, { align: "right" });
const providerDetails = [practiceInfo?.doctor_specialty, practiceInfo?.doctor_license].filter(Boolean).join(" · ");
if (providerDetails) doc.text(providerDetails, pageWidth - 14, 26, { align: "right" });
else doc.text(`Generated: ${formatDateTime(generatedAt)}`, pageWidth - 14, 26, { align: "right" });
if (doctorPhotoDataUrl) {
  try { doc.addImage(doctorPhotoDataUrl, "JPEG", pageWidth - 68, 10, 12, 12); } catch { /* provider text remains */ }
}
