...
const DoctorProfileSection = ({ doctorProfile: propProfile }: DoctorProfileSectionProps) => {
  ...
  const { doctorProfile: profile, loading, refreshAll } = useDoctorData();
  ...
  const toggleSpecialty = (specialty: string, parentSpecialty: string) => {
    ...
    setSelectedSpecialties(prev => {
      ...
      if (prev.length >= 5) { toast.error('Max 5 specialties'); return prev; }
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge className={doctorProfile?.verified ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}>
        ...
        <CardContent className="space-y-4 pt-6">
          <div className="flex items-center gap-6">
            ...
            <Button variant="outline" size="sm" disabled={uploading || isEditingLocked} onClick={() => { 
              ...
              i.onchange = (e) => { const f = (e.target as HTMLInputElement)?.files?.[0]; if (f) handleAvatarUpload(f); }; 
              i.click(); 
            }}>
              ...
              {!useSamePhoneForSms && (
                <div>
                  ...
                  <Input 
                    ...
                    onChange={(e) => setFormData(p => ({ ...p, sms_phone: e.target.value }))}
                    placeholder="+1234567890"
                    disabled={isEditingLocked}
                    className={smsPhoneValidation.isValid ? 'border-green-500' : formData.sms_phone ? 'border-red-500' : ''}
                  />
                  {formData.sms_phone && (
          ...
          {selectedSpecialties.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {selectedSpecialties.map((s, i) => (
                <Badge key={s} variant={i === 0 ? "default" : "secondary"} className="flex items-center gap-1">
            ...
            {filteredMainSpecialties.map(main => {
              ...
              return (
                <div key={main} className="border-b last:border-b-0">
                  ...
                  {expandedSpecialty === main && (
                    <div className="px-4 pb-2 grid grid-cols-2 gap-1 bg-muted/30">
                      {subs.map(sub => {
                        const fullName = `${main} - ${sub}`;
                        return (
                          <label key={sub} className="flex items-center gap-2 p-1 text-sm cursor-pointer">
                            <Checkbox 
                              checked={selectedSpecialties.includes(fullName)} 
                              onCheckedChange={() => toggleSpecialty(sub, main)} 
                              disabled={isEditingLocked} 
                            />
                            {sub}
                          </label>
      ...
      <Card>
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="border rounded-lg max-h-[150px] overflow-y-auto p-2 grid grid-cols-3 gap-1">
            {filteredLanguages.slice(0, 30).map(l => (
              <label key={l} className="flex items-center gap-2 p-1 text-sm cursor-pointer">
        ...
        <CardContent className="space-y-4 pt-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input value={formData.license_number} onChange={(e) => setFormData(p => ({ ...p, license_number: e.target.value }))} disabled={isEditingLocked} />
            </div>
            <div>
              <Label>Experience</Label>
              <Select value={formData.years_experience} onValueChange={(v) => setFormData(p => ({ ...p, years_experience: v }))} disabled={isEditingLocked}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {consultationTypes.map(t => (
                <label key={t} className={`flex items-center gap-2 px-3 py-2 border rounded-lg cursor-pointer ${selectedConsultationTypes.includes(t) ? 'border-primary bg-primary/10' : ''}`}>
          ...
          <div>
            ...
            <Input type="number" value={formData.consultation_fee} onChange={(e) => setFormData(p => ({ ...p, consultation_fee: e.target.value }))} disabled={isEditingLocked} />
          </div>
        </CardContent>
      </Card>
