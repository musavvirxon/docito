...
import type { ReferralEntityType, ReferralType, ReferralPriority, CreateReferralInput } from '@/hooks/useReferrals';

const referralSchema = z.object({
  patient_id: z.string().min(1, 'Patient is required'),
  receiver_type: z.enum(['doctor', 'clinic', 'lab', 'imaging_center', 'pharmacy']),
  receiver_entity_id: z.string().min(1, 'Please select a receiver'),
  referral_type: z.enum(['consultation', 'lab_test', 'imaging_study', 'prescription_fulfillment', 'follow_up_care', 'specialist_referral']),
...
export const CreateReferralDialog = ({
  ...
  // Filter referral types based on receiver type
  const filteredReferralTypes = referralTypeOptions.filter(
    opt => opt.forTypes.includes(selectedReceiverType)
  );

  // Search receivers when type or search term changes
  useEffect(() => {
    const search = async () => {
  ...
  }, [selectedReceiverType, searchTerm]);

  // Reset receiver selection when type changes
  useEffect(() => {
  ...
  const getReceiverSubtext = (receiver: any) => {
    if (selectedReceiverType === 'doctor') {
      return `${receiver.specialty} • ${receiver.practices?.name || 'Independent'}`;
    }
    return `${receiver.city || ''}, ${receiver.country || ''}`;
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a referral for {patientName}
          </DialogDescription>
        </DialogHeader>
                    ...
                    <FormControl>
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="stat" className="text-destructive">STAT</Label>
                        </div>
                      </RadioGroup>
                    </FormControl>
                    <FormMessage />
