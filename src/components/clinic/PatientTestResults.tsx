        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="lab" className="gap-2">
              ...
              {t('testResults.labTab', { count: labOrders.length })}
            </TabsTrigger>
            <TabsTrigger value="imaging" className="gap-2">
              <ScanLine className="h-4 w-4" />
              {t('testResults.imagingTab', { count: imagingOrders.length })}
            </TabsTrigger>
          </TabsList>

          <TabsContent value="lab" className="mt-4">
            {loading ? (
              <div className="py-8 text-center text-muted-foreground">{t('testResults.loading')}</div>
            ) : labOrders.length === 0 ? (
              <div className="py-8 text-center text-muted-foreground">
                <FlaskConical className="h-12 w-12 mx-auto mb-4 opacity-50" />
                <p>{t('testResults.emptyLab')}</p>
              </div>
            ) : (
                      ...
                      <div className="p-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              ...
                              {order.is_abnormal && (
                                <Badge variant="destructive">{t('testResults.abnormal')}</Badge>
                              )}
                            </div>
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                {order.completed_at ? format(new Date(order.completed_at), 'MMM d, yyyy') : t('testResults.na')}
                              </span>
                            </div>
                          </div>
                        ...
                        {order.result_text && (
                          <div className="mt-3 p-3 bg-muted rounded-lg">
                            <p className="text-sm font-medium mb-1">{t('testResults.result')}</p>
                            <p className="text-sm">{order.result_text}</p>
                            {order.reference_range && (
                              <p className="text-xs text-muted-foreground mt-1">
                                {t('testResults.reference', { range: order.reference_range })}
                              </p>
                            )}
                          </div>
                        )}
          ...
          <TabsContent value="imaging" className="mt-4">
            {loading ? (
              ...
              <div className="py-8 text-center text-muted-foreground">
                ...
                <p>{t('testResults.emptyImaging')}</p>
              </div>
            ) : (
              <ScrollArea className="h-[400px]">
                <div className="space-y-4">
                  {imagingOrders.map(order => (
                    <Card key={order.id} className="overflow-hidden">
                      <div className="p-4">
                        ...
                        {order.impression && (
                          <div className="mt-3 p-3 bg-muted rounded-lg">
                            <p className="text-sm font-medium mb-1">{t('testResults.impression')}</p>
                        ...
                        {order.findings && (
                          <div className="mt-2 p-3 bg-muted rounded-lg">
                            <p className="text-sm font-medium mb-1">{t('testResults.findings')}</p>
                        ...
                        <div className="mt-3 flex gap-2">
                          ...
                          {order.result_images && order.result_images.length > 0 && (
                            <Button variant="outline" size="sm">
                              {t('testResults.viewImages', { count: order.result_images.length })}
                            </Button>
                          )}
                        </div>
                      </div>
