...
import { languages as supportedLanguages } from "@/i18n/config";
...
export const SEOHead = ({
  ...
  const langPrefixRegex = new RegExp(`^\\/(${langCodes.join("|")})(?=\\/|$)`, "i");

  useEffect(() => {
    const pathWithoutLang = location.pathname.replace(langPrefixRegex, "") || "/";
    ...
    langCodes.forEach((lang) => {
      ...
      link.setAttribute("href", `${baseUrl}/${lang}${pathWithoutLang}`);
    ...
    xDefault.setAttribute("href", `${baseUrl}/en${pathWithoutLang}`);
